    ...
    <div className="w-full max-w-3xl mx-auto space-y-8">
      ...
      <Card className="bg-gradient-to-r from-green-500 to-green-700 text-white shadow-md">
        <CardHeader className="flex flex-col items-center justify-center py-8">
          ...
          <CardTitle className="text-2xl font-bold text-white text-center">¡Proyecciones Generadas Exitosamente!</CardTitle>
          <p className="mt-2 text-white/90 text-center text-base">Se generaron {files.length} archivos Excel trimestrales</p>
        </CardHeader>
      </Card>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {files.map((file) => (
          <Card key={file.fileName} className="flex flex-col justify-between p-4 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              <FileSpreadsheet className="w-8 h-8 text-green-600" />
              <div>
                <p className="font-medium text-sm break-all">{file.fileName}</p>
                <p className="text-xs text-muted-foreground">{file.quarter}</p>
              </div>
            </div>
            <Button
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-center items-center mt-6">
        <Button
          className="w-full sm:w-auto bg-gradient-to-r from-indigo-600 to-blue-500 text-white font-semibold text-lg px-8 py-3"
          onClick={downloadAll}
        >
        ...
        <Button
          variant="outline"
          className="w-full sm:w-auto text-base px-8 py-3 border-2 border-indigo-500"
          onClick={onReset}
        >
          Nueva Proyección
        </Button>
